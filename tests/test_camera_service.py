...
class TestCameraService(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        self.camera_service.get_object_list = AsyncMock(return_value=[
    ...
    async def test_turn_on_off_devicemgmt_camera(self):
        ...
        self.camera_service._run_action_devicemgmt.assert_awaited_with(self.devicemgmt_camera, "power", "wakeup")

        await self.camera_service.turn_off(self.devicemgmt_camera)
        self.camera_service._run_action_devicemgmt.assert_awaited_with(self.devicemgmt_camera, "power", "sleep")
    ...
    async def test_siren_control_devicemgmt_camera(self):
        ...
        self.camera_service._run_action_devicemgmt.assert_awaited_with(self.devicemgmt_camera, "siren", "siren-on")

        await self.camera_service.siren_off(self.devicemgmt_camera)
        self.camera_service._run_action_devicemgmt.assert_awaited_with(self.devicemgmt_camera, "siren", "siren-off")
    ...
    async def test_floodlight_control_devicemgmt_camera(self):
        ...
        self.camera_service._run_action_devicemgmt.assert_awaited_with(self.devicemgmt_camera, "floodlight", "1")

        await self.camera_service.floodlight_off(self.devicemgmt_camera)
        self.camera_service._run_action_devicemgmt.assert_awaited_with(self.devicemgmt_camera, "floodlight", "0")

    async def test_floodlight_control_bcp_camera(self):
        await self.camera_service.floodlight_on(self.bcp_camera)
        self.camera_service._run_action_devicemgmt.assert_awaited_with(self.bcp_camera, "spotlight", "1")

        await self.camera_service.floodlight_off(self.bcp_camera)
        self.camera_service._run_action_devicemgmt.assert_awaited_with(self.bcp_camera, "spotlight", "0")
    ...
    async def test_notification_control_devicemgmt_camera(self):
        ...
        self.camera_service._set_toggle.assert_awaited_with(self.devicemgmt_camera, DeviceMgmtToggleProps.NOTIFICATION_TOGGLE.value, "1")

        await self.camera_service.turn_off_notifications(self.devicemgmt_camera)
        self.camera_service._set_toggle.assert_awaited_with(self.devicemgmt_camera, DeviceMgmtToggleProps.NOTIFICATION_TOGGLE.value, "0")
    ...
    async def test_motion_detection_control_devicemgmt_camera(self):
        ...
        self.camera_service._set_toggle.assert_awaited_with(self.devicemgmt_camera, DeviceMgmtToggleProps.EVENT_RECORDING_TOGGLE.value, "1")

        await self.camera_service.turn_off_motion_detection(self.devicemgmt_camera)
        self.camera_service._set_toggle.assert_awaited_with(self.devicemgmt_camera, DeviceMgmtToggleProps.EVENT_RECORDING_TOGGLE.value, "0")
    ...
    async def test_motion_detection_control_wco2_camera(self):
        with patch('wyzeapy.services.camera_service.create_pid_pair', side_effect=lambda x, y: (x, y)):
            ...
            self.camera_service._set_property_list.assert_awaited_with(self.wco2_camera, [(PropertyIDs.WCO_MOTION_DETECTION, "1")])

            await self.camera_service.turn_off_motion_detection(self.wco2_camera)
            self.camera_service._set_property_list.assert_awaited_with(self.wco2_camera, [(PropertyIDs.WCO_MOTION_DETECTION, "0")])

    async def test_garage_door_open_close(self):
        await self.camera_service.garage_door_open(self.test_camera)
        self.camera_service._run_action.assert_awaited_with(self.test_camera, "garage_door_trigger")
    ...
    async def test_update_worker_exceptions(self):
        ...
        with patch('wyzeapy.services.camera_service._LOGGER.warning') as mock_warning,\
            ...
            self.assertGreaterEqual(self.camera_service.update.call_count, len(exceptions_to_raise))
            # Check that the warning was called for UnknownApiError
            mock_warning.assert_called_with("The update method detected an UnknownApiError: API Error")
